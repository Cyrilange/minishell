...
void redirect_outfile(char *fil, bool append) {
  ...
  if (append == true) // if ">>"
    outfile_fd =
        open(fil, O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IWUSR); // append
  else                                                               // if ">"
    outfile_fd =
        open(fil, O_WRONLY | O_TRUNC | O_CREAT, S_IRUSR | S_IWUSR); // truncate
    ...
    else // if its not a builtin
    {
      if (!is_path(command[0])) // if program was not sent as an abs path
        get_cmd_path(*envp, &command[0],
                     command[0]); // convert program name to abs path for execve
      ...
      else // parent proc
      {
        ...
        g_status =
            g_status / 256; // idk why waitpid multiplies the errcode by 256
      }
    }
  }
}
